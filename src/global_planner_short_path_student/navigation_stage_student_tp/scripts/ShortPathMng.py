...
class ShortPathMng:
    ...
    def inflate_map(self, map, map_resolution):
        ...
        size_inflation = int(self.inflate_radius/map_resolution)
        ...
        for i in range (len(map)):
            for j in range (0, len(map[i])):
                if map[i][j] == self.MAP_OBSTACLE_VALUE:
                    for h in range (i-size_inflation, i+size_inflation):
                        for k in range (j-size_inflation, j+size_inflation):
                            if (h>=0) and (h<len(map)) and (k>=0) and (k<len(map)):
                                new_inflated_map[h][k] = self.MAP_OBSTACLE_VALUE
